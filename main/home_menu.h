...
extern const uint8_t BATTERY_PIN; // Link to battery pin
...
// Helper function to read battery percentage
int getBatteryPercentage() {
  int mv = analogReadMilliVolts(BATTERY_PIN);
  float voltage = (mv / 1000.0) * 2.0;   // 10k + 10k hardware voltage divider ratio
  int percent = (voltage - 3.0) * 100 / 1.2; // Map 3.0V-4.2V to 0-100%
...
void drawHomeMenuGrid() {
  ...
  // Left-aligned header text title
  display.setCursor(0, 0);
  display.println("SELECT APP");
  
  // --- ADDED: Real-time battery indicator on top right ---
  int batPercent = getBatteryPercentage();
  display.setCursor(95, 0); // Position at right edge of OLED canvas
  display.print(batPercent);
  display.println("%");
  
  // Divider line beneath system status row
  display.drawLine(0, 10, 127, 10, SSD1306_WHITE);
  
  // Shifting dynamic viewport window tracking logic
  int windowStartOffset = 0;
  if (menuSelectedAppIndex >= 4) {
    windowStartOffset = menuSelectedAppIndex - 3; 
  }

  // Render viewport rows cleanly
  for (int i = 0; i < 4; i++) {
    int appIndex = windowStartOffset + i;
    if (appIndex >= totalAppsCount) break; 

    int verticalTextOffset = 15 + (i * 12); 
